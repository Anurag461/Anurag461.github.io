...
A pseudorandom code (PRC) watermark hides a codeword inside the bits of a sampled response. Detection is a soft majority vote over a sparse parity-check matrix `P`: count how many parity checks of `Px XOR Pz` are unsatisfied, and output 1 if that count falls below `(0.5 - zeta)*r`. 
